...
class OnMuhasebe:
        ...
        try:
            ...
            if is_unique:
                try:
                    ...
                    print(f"UYARI: UNIQUE INDEX oluşturulurken hata oluştu: {e}. Bu genellikle '{table_name}.{column_name}' sütununda zaten yinelenen veri olduğu anlamına gelir.")
                except Exception as e:
    ...
    def musteri_getir_by_id(self, musteri_id):
        ...
        if result:
            ...
            print(f"DEBUG: Müşteri adı: {result['ad']}, Kodu: {result['kod']}")
        return result
    ...
    def get_tedarikci_net_bakiye(self, tedarikci_id, tarih_filtresi=None):
        net_bakiye = 0.0
        try:
            # Cari hareketler tablosuna faturalar tablosunu JOIN ederek odeme_turu bilgisini alıyoruz.
            query = """
            ...
            for hareket in self.c.fetchall():
                ...
                odeme_turu = hareket['odeme_turu'] # f.odeme_turu'ndan gelen değer (None olabilir)

                # Mantık:
                # 1. Açık hesap faturalar (ve onların ödemeleri) cari bakiyeyi etkiler.
                # 2. Manuel veresiye borçlar (manuel ödemeleri ile kapanırsa) cari bakiyeyi etkiler.
                # 3. PEŞİN FATURALAR (ve onların ödemeleri) cari bakiyeye NET SIFIR etki eder.

                if referans_tip == self.KAYNAK_TIP_FATURA and odeme_turu == self.ODEME_TURU_ACIK_HESAP:
                    # Açık hesap alış faturası: Biz borçlandık (borç)
                    net_bakiye += tutar
                elif referans_tip == self.KAYNAK_TIP_IADE_FATURA and odeme_turu == self.ODEME_TURU_ACIK_HESAP:
                    # Açık hesap alış iade faturası: Borcumuz azaldı
                    net_bakiye -= tutar
                elif referans_tip == self.KAYNAK_TIP_ODEME: # Manuel ödemeler
                    net_bakiye -= tutar
                elif referans_tip == self.KAYNAK_TIP_VERESIYE_BORC_MANUEL: # Manuel veresiye borçlar
                    net_bakiye += tutar
                # Peşin faturalar ve peşin iadeler net bakiyeyi ETKİLEMEZ.
    ...
    def get_musteri_net_bakiye(self, musteri_id, tarih_filtresi=None):
        ...
        try:
            # Cari hareketler tablosuna faturalar tablosunu JOIN ederek odeme_turu bilgisini alıyoruz.
            # Bu sayede hareketin peşin bir fatura kaynaklı olup olmadığını anlayabiliriz.
            query = """
                SELECT ch.islem_tipi, ch.tutar, ch.referans_tip, f.odeme_turu
                FROM cari_hareketler ch
                LEFT JOIN faturalar f ON ch.referans_id = f.id
            ...
            for hareket in self.c.fetchall():
                ...
                # Mantık:
                # 1. Açık hesap faturalar (ve onların tahsilatları) cari bakiyeyi etkiler.
                # 2. Manuel veresiye borçlar (manuel tahsilatları ile kapanırsa) cari bakiyeyi etkiler.
                # 3. PEŞİN FATURALAR (ve onların tahsilatları) cari bakiyeye NET SIFIR etki eder, bu yüzden burada doğrudan bakiyeyi değiştirmeyiz.
                #    Aynı şekilde peşin iade faturaları da.

                if referans_tip == self.KAYNAK_TIP_FATURA and odeme_turu == self.ODEME_TURU_ACIK_HESAP:
                    # Açık hesap satış faturası: Müşteri borçlandı (alacak)
                    net_bakiye += tutar
                elif referans_tip == self.KAYNAK_TIP_IADE_FATURA and odeme_turu == self.ODEME_TURU_ACIK_HESAP:
                    # Açık hesap satış iade faturası: Müşteri alacaklandı (borç azaldı)
                    net_bakiye -= tutar
                elif referans_tip == self.KAYNAK_TIP_TAHSILAT: # Manuel tahsilatlar
                    net_bakiye -= tutar
                elif referans_tip == self.KAYNAK_TIP_VERESIYE_BORC_MANUEL: # Manuel veresiye borçlar
                    net_bakiye += tutar
                # NOT: Peşin faturalar (FATURA_SATIS_PESIN ve FATURA_ALIS_PESIN) ve onların tahsilat/ödeme hareketleri
                # burada doğrudan net_bakiye'yi değiştirmez, çünkü net etkileri sıfırdır.
                # Eğer islem_tipi kontrolü gerekiyorsa, referans_tip kontrolü ile birlikte yapılmalı.
    ...
    def cari_hesap_ekstresi_al(self, cari_tip, cari_id, baslangic_tarih_str, bitis_tarih_str):
        # <<< DEĞİŞİKLİK: JOIN mantığı ve sütunlar düzeltildi, devir bakiye hesaplaması iyileştirildi >>>
        try:
            # Devreden bakiyeyi doğrudan hesaplayan metodu kullanıyoruz
            devreden_bakiye = self._get_cari_devir_bakiye(cari_tip, cari_id, baslangic_tarih_str)
            
            # Ana sorgu: LEFT JOIN artık hem FATURA hem de IADE_FATURA için çalışacak
            query = """
                SELECT
                    ...
                    f.fatura_no, f.odeme_turu, f.vade_tarihi
                FROM cari_hareketler ch
                LEFT JOIN faturalar f ON ch.referans_id = f.id AND (ch.referans_tip = 'FATURA' OR ch.referans_tip = 'İADE_FATURA')
            ...
            return hareketler_listesi, devreden_bakiye, 0.0, 0.0 # Son 0.0 değerleri artık kullanılmıyor, uyumluluk için duruyor
        except Exception as e:
            logging.error(f"cari_hesap_ekstresi_al içinde genel hata: {e}\n{traceback.format_exc()}")
            return [], 0.0, 0.0, 0.0
                    
    def get_cari_ozet_bilgileri(self, cari_id, cari_tip):
        ozet = { "toplam_satis": 0.0, "toplam_alis": 0.0, "toplam_tahsilat": 0.0, "toplam_odeme": 0.0, "net_bakiye": 0.0, "vadesi_gelmis_borc_alacak": 0.0, "vadesi_gelecek_borc_alacak": 0.0, "donem_basi_bakiye": 0.0, "donem_sonu_bakiye": 0.0, "donem_toplam_borc_hareketi": 0.0, "donem_toplam_alacak_hareketi": 0.0 }
        try:
            # Dönem başı bakiyeyi almak için (bugünden önceki tüm hareketler)
            # Bu çağrılar artık sadece açık hesap bakiyesini döndürüyor.
            ...
            # Tarih aralığını belirle (şimdilik tüm zamanlar)
            baslangic_tarih = "1900-01-01" # Çok eski bir tarih
            bitis_tarih = datetime.now().strftime('%Y-%m-%d')

            # Dönem içi borç ve alacak hareketlerini hesapla (BU KISIM DA SADECE AÇIK HESAP HAREKETLERİNİ DİKKATE ALMALI)
            # Çünkü özet bölümü net borcu/alacağı gösterecek.
            # Ancak "Toplam Alacak Hareketi" ve "Toplam Borç Hareketi" faturaların brüt tutarını yansıtmalı (peşin veya açık).
            # Bu yüzden sorguyu doğrudan faturalardan ve manuel veresiye borçlardan alacak/borç yaratıcı olarak güncelleyeceğiz.

            if cari_tip == 'MUSTERI':
                # Toplam Alacak Hareketi: SATIŞ faturalarından gelen toplam tutar
                self.c.execute("SELECT SUM(toplam_kdv_dahil) FROM faturalar WHERE cari_id = ? AND tip IN ('SATIŞ')", (cari_id,))
                ozet["donem_toplam_alacak_hareketi"] = self.c.fetchone()[0] or 0.0
                
                # Toplam Borç Hareketi: TAHSİLAT işlemlerinden gelen toplam tutar
                self.c.execute("SELECT SUM(tutar) FROM cari_hareketler WHERE cari_id = ? AND cari_tip = 'MUSTERI' AND islem_tipi = 'TAHSILAT'", (cari_id,))
                ozet["donem_toplam_borc_hareketi"] = self.c.fetchone()[0] or 0.0

                # Toplam Tahsilat/Ödeme: Sadece tahsilat işlemlerini doğrudan topluyoruz.
                # Daha önce tahsilat, gelir/gider üzerinden geliyordu. Şimdi cari hareketlerden.
                # Bu zaten ozet["donem_toplam_borc_hareketi"] ile aynı mantıkta.
                ozet["toplam_tahsilat"] = ozet["donem_toplam_borc_hareketi"]
                
                # Net bakiyeyi ve vadesi geçen/gelecek borçları güncel tarihe göre al
                ...
                # Toplam Alacak Hareketi (Tedarikçi için "Borç Yaratıcı"): ALIŞ faturalarından gelen toplam tutar
                self.c.execute("SELECT SUM(toplam_kdv_dahil) FROM faturalar WHERE cari_id = ? AND tip IN ('ALIŞ')", (cari_id,))
                ozet["donem_toplam_alacak_hareketi"] = self.c.fetchone()[0] or 0.0 # Tedarikçiye olan borcumuzun artışı

                # Toplam Borç Hareketi (Tedarikçi için "Ödeme Yaratıcı"): ÖDEME işlemlerinden gelen toplam tutar
                self.c.execute("SELECT SUM(tutar) FROM cari_hareketler WHERE cari_id = ? AND cari_tip = 'TEDARIKCI' AND islem_tipi = 'ODEME'", (cari_id,))
                ozet["donem_toplam_borc_hareketi"] = self.c.fetchone()[0] or 0.0 # Tedarikçiye yapılan ödemeler

                # Toplam Tahsilat/Ödeme: Ödeme işlemlerini doğrudan topluyoruz.
                ozet["toplam_odeme"] = ozet["donem_toplam_borc_hareketi"]
