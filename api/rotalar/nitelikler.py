...
def create_kategori(
    ...
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunKategori yerine modeller.UrunKategori kullanıldı.
    db_kategori = modeller.UrunKategori(**kategori.model_dump(), kullanici_id=current_user.id)
    db.add(db_kategori)
    db.commit()
...
def read_kategoriler(
    ...
    # KRİTİK DÜZELTME: semalar.UrunKategori yerine modeller.UrunKategori kullanıldı.
    query = db.query(modeller.UrunKategori).filter(modeller.UrunKategori.kullanici_id == current_user.id)
    if arama:
...
def read_kategori(
    ...
    # KRİTİK DÜZELTME: semalar.UrunKategori yerine modeller.UrunKategori kullanıldı.
    kategori = db.query(modeller.UrunKategori).filter(modeller.UrunKategori.id == kategori_id, modeller.UrunKategori.kullanici_id == current_user.id).first()
    if not kategori:
...
def update_kategori(
    kategori_id: int, 
    kategori: modeller.UrunKategoriUpdate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunKategori yerine modeller.UrunKategori kullanıldı.
    db_kategori = db.query(modeller.UrunKategori).filter(modeller.UrunKategori.id == kategori_id, modeller.UrunKategori.kullanici_id == current_user.id).first()
    if not db_kategori:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Kategori bulunamadı")
...
def delete_kategori(
    ...
    if not db_kategori:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Kategori bulunamadı")
    # KRİTİK DÜZELTME: İlişkili Stok kontrolünde semalar.Stok yerine modeller.Stok kullanıldı.
    if db.query(modeller.Stok).filter(modeller.Stok.kategori_id == kategori_id, modeller.Stok.kullanici_id == current_user.id).first():
...
def create_marka(
    marka: modeller.UrunMarkaCreate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunMarka yerine modeller.UrunMarka kullanıldı.
    db_marka = modeller.UrunMarka(**marka.model_dump(), kullanici_id=current_user.id)
    db.add(db_marka)
    db.commit()
    db.refresh(db_marka)
    return db_marka
...
def read_markalar(
    ...
    # KRİTİK DÜZELTME: semalar.UrunMarka yerine modeller.UrunMarka kullanıldı.
    query = db.query(modeller.UrunMarka).filter(modeller.UrunMarka.kullanici_id == current_user.id)
    if arama:
...
def read_marka(
    ...
    # KRİTİK DÜZELTME: semalar.UrunMarka yerine modeller.UrunMarka kullanıldı.
    marka = db.query(modeller.UrunMarka).filter(modeller.UrunMarka.id == marka_id, modeller.UrunMarka.kullanici_id == current_user.id).first()
    if not marka:
...
def update_marka(
    marka_id: int, 
    marka: modeller.UrunMarkaUpdate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunMarka yerine modeller.UrunMarka kullanıldı.
    db_marka = db.query(modeller.UrunMarka).filter(modeller.UrunMarka.id == marka_id, modeller.UrunMarka.kullanici_id == current_user.id).first()
    if not db_marka:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Marka bulunamadı")
...
def delete_marka(
    ...
    if not db_marka:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Marka bulunamadı")
    # KRİTİK DÜZELTME: İlişkili Stok kontrolünde semalar.Stok yerine modeller.Stok kullanıldı.
    if db.query(modeller.Stok).filter(modeller.Stok.marka_id == marka_id, modeller.Stok.kullanici_id == current_user.id).first():
...
def create_urun_grubu(
    urun_grubu: modeller.UrunGrubuCreate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunGrubu yerine modeller.UrunGrubu kullanıldı.
    db_urun_grubu = modeller.UrunGrubu(**urun_grubu.model_dump(), kullanici_id=current_user.id)
...
def read_urun_gruplari(
    ...
    # KRİTİK DÜZELTME: semalar.UrunGrubu yerine modeller.UrunGrubu kullanıldı.
    query = db.query(modeller.UrunGrubu).filter(modeller.UrunGrubu.kullanici_id == current_user.id)
    if arama:
...
def read_urun_grubu(
    ...
    # KRİTİK DÜZELTME: semalar.UrunGrubu yerine modeller.UrunGrubu kullanıldı.
    urun_grubu = db.query(modeller.UrunGrubu).filter(modeller.UrunGrubu.id == urun_grubu_id, modeller.UrunGrubu.kullanici_id == current_user.id).first()
    if not urun_grubu:
...
def update_urun_grubu(
    urun_grubu_id: int, 
    urun_grubu: modeller.UrunGrubuUpdate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunGrubu yerine modeller.UrunGrubu kullanıldı.
    db_urun_grubu = db.query(modeller.UrunGrubu).filter(modeller.UrunGrubu.id == urun_grubu_id, modeller.UrunGrubu.kullanici_id == current_user.id).first()
...
def delete_urun_grubu(
    ...
    if not db_urun_grubu:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Ürün grubu bulunamadı")
    # KRİTİK DÜZELTME: İlişkili Stok kontrolünde semalar.Stok yerine modeller.Stok kullanıldı.
    if db.query(modeller.Stok).filter(modeller.Stok.urun_grubu_id == urun_grubu_id, modeller.Stok.kullanici_id == current_user.id).first():
...
def create_urun_birimi(
    urun_birimi: modeller.UrunBirimiCreate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunBirimi yerine modeller.UrunBirimi kullanıldı.
    db_urun_birimi = modeller.UrunBirimi(**urun_birimi.model_dump(), kullanici_id=current_user.id)
...
def read_urun_birimleri(
    ...
    # KRİTİK DÜZELTME: semalar.UrunBirimi yerine modeller.UrunBirimi kullanıldı.
    query = db.query(modeller.UrunBirimi).filter(modeller.UrunBirimi.kullanici_id == current_user.id)
    if arama:
...
def read_urun_birimi(
    urun_birimi_id: int, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunBirimi yerine modeller.UrunBirimi kullanıldı.
    urun_birimi = db.query(modeller.UrunBirimi).filter(modeller.UrunBirimi.id == urun_birimi_id, modeller.UrunBirimi.kullanici_id == current_user.id).first()
    if not urun_birimi:
...
def update_urun_birimi(
    urun_birimi_id: int, 
    urun_birimi: modeller.UrunBirimiUpdate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.UrunBirimi yerine modeller.UrunBirimi kullanıldı.
    db_urun_birimi = db.query(modeller.UrunBirimi).filter(modeller.UrunBirimi.id == urun_birimi_id, modeller.UrunBirimi.kullanici_id == current_user.id).first()
...
def delete_urun_birimi(
    ...
    if not db_urun_birimi:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Ürün birimi bulunamadı")
    # KRİTİK DÜZELTME: İlişkili Stok kontrolünde semalar.Stok yerine modeller.Stok kullanıldı.
    if db.query(modeller.Stok).filter(modeller.Stok.birim_id == urun_birimi_id, modeller.Stok.kullanici_id == current_user.id).first():
...
def create_ulke(
    ulke: modeller.UlkeCreate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.Ulke yerine modeller.Ulke kullanıldı.
    db_ulke = modeller.Ulke(**ulke.model_dump(), kullanici_id=current_user.id)
    db.add(db_ulke)
    db.commit()
    db.refresh(db_ulke)
    return db_ulke
...
def read_ulkeler(
    ...
    # KRİTİK DÜZELTME: semalar.Ulke yerine modeller.Ulke kullanıldı.
    query = db.query(modeller.Ulke).filter(modeller.Ulke.kullanici_id == current_user.id)
    if arama:
...
def read_ulke(
    ...
    # KRİTİK DÜZELTME: semalar.Ulke yerine modeller.Ulke kullanıldı.
    ulke = db.query(modeller.Ulke).filter(modeller.Ulke.id == ulke_id, modeller.Ulke.kullanici_id == current_user.id).first()
    if not ulke:
...
def update_ulke(
    ulke_id: int, 
    ulke: modeller.UlkeUpdate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.Ulke yerine modeller.Ulke kullanıldı.
    db_ulke = db.query(modeller.Ulke).filter(modeller.Ulke.id == ulke_id, modeller.Ulke.kullanici_id == current_user.id).first()
    if not db_ulke:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Ülke bulunamadı")
...
def delete_ulke(
    ...
    if not db_ulke:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Ülke bulunamadı")
    # KRİTİK DÜZELTME: İlişkili Stok kontrolünde semalar.Stok yerine modeller.Stok kullanıldı.
    if db.query(modeller.Stok).filter(modeller.Stok.mense_id == ulke_id, modeller.Stok.kullanici_id == current_user.id).first():
...
def create_gelir_siniflandirma(
    siniflandirma: modeller.GelirSiniflandirmaCreate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.GelirSiniflandirma yerine modeller.GelirSiniflandirma kullanıldı.
    db_siniflandirma = modeller.GelirSiniflandirma(**siniflandirma.model_dump(), kullanici_id=current_user.id)
...
def read_gelir_siniflandirmalari(
    ...
    # KRİTİK DÜZELTME: semalar.GelirSiniflandirma yerine modeller.GelirSiniflandirma kullanıldı.
    query = db.query(modeller.GelirSiniflandirma).filter(modeller.GelirSiniflandirma.kullanici_id == current_user.id)
    if id:
...
def read_gelir_siniflandirma(
    siniflandirma_id: int, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.GelirSiniflandirma yerine modeller.GelirSiniflandirma kullanıldı.
    siniflandirma = db.query(modeller.GelirSiniflandirma).filter(modeller.GelirSiniflandirma.id == siniflandirma_id, modeller.GelirSiniflandirma.kullanici_id == current_user.id).first()
...
def update_gelir_siniflandirma(
    siniflandirma_id: int, 
    siniflandirma: modeller.GelirSiniflandirmaUpdate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.GelirSiniflandirma yerine modeller.GelirSiniflandirma kullanıldı.
    db_siniflandirma = db.query(modeller.GelirSiniflandirma).filter(modeller.GelirSiniflandirma.id == siniflandirma_id, modeller.GelirSiniflandirma.kullanici_id == current_user.id).first()
...
def delete_gelir_siniflandirma(
    siniflandirma_id: int, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    ...
    if not db_siniflandirma:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Gelir sınıflandırması bulunamadı")
    # KRİTİK DÜZELTME: İlişkili GelirGider kontrolünde semalar.GelirGider yerine modeller.GelirGider kullanıldı.
    if db.query(modeller.GelirGider).filter(modeller.GelirGider.gelir_siniflandirma_id == siniflandirma_id, modeller.GelirGider.kullanici_id == current_user.id).first():
...
def create_gider_siniflandirma(
    siniflandirma: modeller.GiderSiniflandirmaCreate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.GiderSiniflandirma yerine modeller.GiderSiniflandirma kullanıldı.
    db_siniflandirma = modeller.GiderSiniflandirma(**siniflandirma.model_dump(), kullanici_id=current_user.id)
    db.add(db_siniflandirma)
    db.commit()
    db.refresh(db_siniflandirma)
    return db_siniflandirma
...
def read_gider_siniflandirmalari(
    ...
    # KRİTİK DÜZELTME: semalar.GiderSiniflandirma yerine modeller.GiderSiniflandirma kullanıldı.
    query = db.query(modeller.GiderSiniflandirma).filter(modeller.GiderSiniflandirma.kullanici_id == current_user.id)
    if id:
...
def read_gider_siniflandirma(
    siniflandirma_id: int, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.GiderSiniflandirma yerine modeller.GiderSiniflandirma kullanıldı.
    siniflandirma = db.query(modeller.GiderSiniflandirma).filter(modeller.GiderSiniflandirma.id == siniflandirma_id, modeller.GiderSiniflandirma.kullanici_id == current_user.id).first()
    if not siniflandirma:
...
def update_gider_siniflandirma(
    siniflandirma_id: int, 
    siniflandirma: modeller.GiderSiniflandirmaUpdate, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    # KRİTİK DÜZELTME: semalar.GiderSiniflandirma yerine modeller.GiderSiniflandirma kullanıldı.
    db_siniflandirma = db.query(modeller.GiderSiniflandirma).filter(modeller.GiderSiniflandirma.id == siniflandirma_id, modeller.GiderSiniflandirma.kullanici_id == current_user.id).first()
    if not db_siniflandirma:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Gider sınıflandırması bulunamadı")
    for key, value in siniflandirma.model_dump(exclude_unset=True).items():
        setattr(db_siniflandirma, key, value)
...
def delete_gider_siniflandirma(
    siniflandirma_id: int, 
    db: Session = Depends(get_db), 
    current_user: modeller.KullaniciRead = Depends(guvenlik.get_current_user) # KRİTİK DÜZELTME: Tip ve JWT
):
    ...
    if not db_siniflandirma:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Gider sınıflandırması bulunamadı")
    # KRİTİK DÜZELTME: İlişkili GelirGider kontrolünde semalar.GelirGider yerine modeller.GelirGider kullanıldı.
    if db.query(modeller.GelirGider).filter(modeller.GelirGider.gider_siniflandirma_id == siniflandirma_id, modeller.GelirGider.kullanici_id == current_user.id).first():
