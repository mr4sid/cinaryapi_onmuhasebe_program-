...
def read_gelir_siniflandirmalari(skip: int = 0, limit: int = 1000, arama: str = Query(None), db: Session = Depends(get_db)):
    query = db.query(semalar.GelirSiniflandirma)
    if arama:
        query = query.filter(semalar.GelirSiniflandirma.ad.ilike(f"%{arama}%"))
...
def read_gider_siniflandirmalari(skip: int = 0, limit: int = 1000, arama: str = Query(None), db: Session = Depends(get_db)):
    query = db.query(semalar.GiderSiniflandirma)
    if arama:
        query = query.filter(semalar.GiderSiniflandirma.ad.ilike(f"%{arama}%"))
    siniflandirmalar = query.offset(skip).limit(limit).all()
    total_count = query.count()
